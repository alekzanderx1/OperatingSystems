...
/** 
    Part 1: The entries are getting lost during insertion due to race condition, hence added a mutex lock
            in order to restrict only one thread to insert at a time ie the insertion part is
            a critical section.

    Part 2: Changed mutex to spin lock in the parallel_spin.c source file.
            With Single Mutex: 1 thread: 0.0105s | 4 thread: 0.0271s | 8 thread: 0.0255s | 12 thread: 0.0226s
            With Spin:         1 thread: 0.0085s | 4 thread: 0.0297s | 8 thread: 0.0589s | 12 thread: 0.0820s
            This shows the using spin lock is slower than use of mutex with multiple threads in our case, 
            this is because mutex yeilds the CPU by going to sleep if lock is already acquired
            whereas spin lock does busy waiting and hence hogs up the CPU cycles instead of yeilding.
            This specially becomes a problem when number of threads increase and most of them are busy waiting.

    Part 3: No changes required here as the lock is only required for insertion which was running into race condition.
            Retrival is more or less running in a single instruction and does not have the same issue.

    Part 4: Updated the code to add an array of locks, one for each bucket. This way a thread is blocked 
            only when another thread is trying to insert in that bucket otherwise insertion can run in parallel.
